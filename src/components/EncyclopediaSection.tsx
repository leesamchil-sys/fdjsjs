import { useState, useEffect, useMemo, useCallback, useRef, SyntheticEvent, useReducer } from 'react';
import { 
  Bird as BirdIcon,
  Search,
  SearchX,
...
// Types and Reducer for refactoring
...
function encyclopediaReducer(state: EncyclopediaState, action: EncyclopediaAction): EncyclopediaState {
  switch (action.type) {
...
import { EncyclopediaItemList } from './EncyclopediaItemList';
...
export default function EncyclopediaSection({
  ...
  onLocationClick,
  highlightedItemName,
  onIngredientModalChange
}: EncyclopediaSectionProps) {

  // Auto scroll and highlight selected card when choosing from Map has been removed as per request to avoid background list jumping


  // Listen for filter-location-from-map event to filter detailed locations on main section
  useEffect(() => {
    const handleFilterFromMap = (e: Event) => {
      const customEvent = e as CustomEvent;
      if (!customEvent.detail) return;
      const { locationName, category } = customEvent.detail;
      
      let targetData: any[] = [];
      if (category === 'birds') targetData = birds;
      else if (category === 'insects') targetData = insects;
      else if (category === 'fishing') targetData = fish;
      
      let foundMain: string | null = null;
      if (targetData) {
        for (const item of targetData) {
          if (item.locations && item.locations.includes(locationName)) {
            foundMain = item.category || '기타';
            break;
          }
        }
      }
      
      setSelectedMainLocation(foundMain);
      setSelectedMobileMainLocation(foundMain);
      setSelectedLocations([locationName]);
      setIncludeCommon(false);
    };

    window.addEventListener('filter-location-from-map', handleFilterFromMap);
    return () => {
      window.removeEventListener('filter-location-from-map', handleFilterFromMap);
    };
  }, [birds, insects, fish]);

  // Isolated Filter States
  const [inputValue, setInputValue] = useState('');
  // const [searchQuery, setSearchQuery] = useState(''); // REPLACED by reducer

  // ...
  
  const [state, dispatch] = useReducer(encyclopediaReducer, {
    searchQuery: '',
    favorites: (() => {
        try {
  ...
  const { searchQuery, favorites } = state;

  // Debounce search input to avoid laggy keystrokes
  useEffect(() => {
  ...
  // Synchronize inputValue when searchQuery is cleared externally
  useEffect(() => {
    if (searchQuery === '') {
      setInputValue('');
    }
  ...
  const [selectedIngredientForModal, setSelectedIngredientForModal] = useState<string | null>(null);
  const [isIngredientModalOpen, setIsIngredientModalOpen] = useState(false);

  useEffect(() => {
    onIngredientModalChange?.(isIngredientModalOpen);
  }, [isIngredientModalOpen, onIngredientModalChange]);
  ...
  const handleOpenIngredientModal = useCallback((ingName: string) => {
    setSelectedIngredientForModal(ingName);
    setIsIngredientModalOpen(true);
  }, []);

  const memoizedToggleCompletion = useCallback((id: string, cat?: Category) => toggleCompletion(id, cat), [toggleCompletion]);
  const memoizedToggleMaster = useCallback((id: string, cat?: Category) => toggleMaster(id, cat), [toggleMaster]);
  const memoizedToggleFavorite = useCallback((id: string) => toggleFavorite(id), [toggleFavorite]);
  const memoizedHandleRate = useCallback((id: string, name: string, rating: number) => handleRate(id, name, rating), [handleRate]);

  const isInitialMount = useRef(true);
  const isCategoryChanging = useRef(false);
  ...
  useEffect(() => {
    if (prevCategoryRef.current !== activeCategory) {
      ...
      isCategoryChanging.current = true;
      const timer = setTimeout(() => {
        isCategoryChanging.current = false;
      }, 50);
  ...
  const [isKstInfoOpen, setIsKstInfoOpen] = useState(false);
  const recSettingsRef = useRef<HTMLDivElement>(null);
  const kstInfoRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (kstInfoRef.current && !kstInfoRef.current.contains(event.target as Node)) {
        setIsKstInfoOpen(false);
      }
    }
  ...
  const masterIds = useMemo(() => {
    if (activeCategory === 'birds') return masterBirdIds;
    if (activeCategory === 'insects') return masterInsectIds;
    if (activeCategory === 'fishing') return masterFishIds;
    if (activeCategory === 'cooking') return masterFoodIds;
    if (activeCategory === 'ocean_cleaning') return masterOceanCleaningIds;
    return new Set<string>();
  }, [activeCategory, masterBirdIds, masterInsectIds, masterFishIds, masterFoodIds, masterOceanCleaningIds]);
  ...
  const matchesWeather = (itemWeather: string, contextWeather: GameWeather) => {
    ...
    const actualContext = (normalizedContext === 'Heatwave' || normalizedContext === 'Meteor') ? 'Clear' : normalizedContext;
    ...
    const filtered = (visibleData as any[]).filter(item => {
      ...
      if (isCompleted) {
        if (recExcludeCompleted) {
          ...
          const isStarIncomplete = recIncludeUnratedStars && Math.min(rating, maxStars) < maxStars;
          const isMasterIncomplete = recIncludeUncompletedMaster && !isMaster && !item.excludeFromMaster;
  ...
  const filteredItems = useMemo(() => {
    const items = filteredBySearch.filter(item => {
      ...
      if (item.seasonId && !activeSeasonIds.includes(item.seasonId)) {
        return false;
      }

      if (activeCategory !== 'ocean_cleaning' && isSeasonFilterEnabled && selectedSeasonFilters.length > 0) {
  ...
  // Infinite scroll effect to load more items smoothly
  useEffect(() => {
    const handleScroll = () => {
      if (window.innerHeight + window.scrollY >= document.documentElement.scrollHeight - 600) {
        setVisibleLimit(prev => Math.min(prev + 24, filteredItems.length));
      }
    };
    window.addEventListener('scroll', handleScroll, { passive: true });
  ...
  return (
    <>
          ...
          {currentGameWeather !== 'Unknown' && (
            <div className="mb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-neutral-900 dark:text-stone-200 flex items-center gap-2">
                  <span>지금 잡을 수 있는 {activeCategory === 'birds' ? '새' : activeCategory === 'insects' ? '곤충' : activeCategory === 'ocean_cleaning' ? '도감' : '물고기'}</span>
                  
                  {/* KST 기준 안내 아이콘 버튼 */}
                  <div className="relative inline-block" ref={kstInfoRef}>
                    <button
                      type="button"
                      onClick={() => setIsKstInfoOpen(!isKstInfoOpen)}
                      className="inline-flex items-center justify-center p-1 text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 transition-colors cursor-pointer"
                      title="한국 시간(KST) 기준 안내"
                    >
                    ...
                    <AnimatePresence>
                      {isKstInfoOpen && (
                        <motion.div
                          ...
                          className="absolute left-0 mt-2 w-64 sm:w-72 rounded-2xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 shadow-xl p-3.5 z-[300] text-xs text-stone-700 dark:text-stone-300 font-sans"
                        >
                          <div className="flex items-center justify-between mb-1.5 pb-1.5 border-b border-stone-100 dark:border-stone-800">
                            <span className="font-extrabold text-stone-900 dark:text-stone-100 flex items-center gap-1">
                              <Clock className="h-3.5 w-3.5 text-amber-500" />
                              한국 표준시(KST) 안내
                            </span>
                            <button 
                              type="button" 
                              onClick={() => setIsKstInfoOpen(false)}
                              className="text-stone-400 hover:text-stone-700 dark:hover:text-stone-200 cursor-pointer"
                            >
                              <X className="h-3.5 w-3.5" />
                            </button>
                          </div>
                          <p className="leading-relaxed text-[11.5px] text-stone-600 dark:text-stone-400">
                            지금 잡을 수 있는 추천 영역과 출현 시간대/변경 주기는 서버 표준인 <strong className="text-amber-600 dark:text-amber-400 font-bold">한국 시간(KST, UTC+9)</strong>을 기준으로 작동합니다.
                          </p>
                        </motion.div>
                      )}
                    </AnimatePresence>
          ...
          {currentGameWeather === 'Unknown' ? (
            ...
            <div ref={recScrollRef} className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 px-2 sm:px-4 lg:px-6 scroll-pl-2 sm:scroll-pl-4 snap-x snap-mandatory scroll-smooth select-none">
              {recommendedItems.map((item: any) => {
                ...
                return (
                  <div 
                    key={`rec-${activeCategory}-${item.id}`} 
                    className="min-w-[255px] xs:min-w-[265px] sm:min-w-[280px] snap-start snap-always p-1.5"
                  >
                    <ItemCard 
                      item={item} 
                      type={activeCategory}
              ...
              <div className="p-2 sm:p-4 flex items-center gap-2 sm:gap-3 w-full">
                <div className="relative flex-1 group overflow-hidden search-bar-container">
                  <Search className={cn(
                    ...
                    inputValue ? "text-slate-900 dark:text-stone-100" : "text-neutral-400 dark:text-stone-550 group-focus-within:text-slate-900 dark:group-focus-within:text-stone-100"
                  )} />
                  <input 
                    type="text" 
                  ...
                  {inputValue && (
                    <button
                      onClick={() => { setInputValue(''); dispatch({ type: 'SET_SEARCH_QUERY', payload: '' }); }}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === 'favorites'
                        ...
                        : "bg-white dark:bg-stone-900 text-stone-500 hover:text-stone-700 dark:text-stone-400 dark:hover:text-stone-200 border border-stone-300/70 dark:border-stone-800 hover:border-stone-400 dark:hover:border-stone-700 shadow-sm"
                    )}
                  >
                                          ...
                                          <div className="flex items-center gap-1.5 overflow-hidden shrink-0">
                                            {selectedLocations.length > 0 && (
                                              <button
                                                ...
                                                className={cn(
                                                  "px-2 py-1 rounded-lg text-[9px] font-black transition-all border cursor-pointer whitespace-nowrap shrink-0 flex items-center gap-1 shadow-xs",
                                                  !includeCommon
                                                    ? "bg-blue-600 border-blue-500 text-white shadow-md ring-1 ring-blue-400/40"
                                                    : "bg-stone-100 border-stone-300 text-stone-500 dark:bg-stone-800 dark:border-stone-700 dark:text-stone-400 font-bold hover:bg-stone-200 dark:hover:bg-stone-750"
                                                )}
                                              >
                                                <span className={cn(
                                                  "w-1.5 h-1.5 rounded-full shrink-0 transition-all",
                                                  !includeCommon ? "bg-emerald-300 animate-pulse shadow-[0_0_6px_rgba(110,231,183,0.9)]" : "bg-stone-400 dark:bg-stone-600"
                                                )} />
                                          ...
                                          <div className="flex items-center gap-1.5 overflow-hidden shrink-0">
                                            {selectedLocations.length > 0 && (
                                              ...
                                              >
                                                ...
                                                <span>{`이 지역의 ${categoryNoun}만`}</span>
                                              </button>
                                            )}
                                            <button onClick={() => setSelectedLocations([])} className="text-[9px] font-bold text-blue-500 cursor-pointer shrink-0">초기화</button>
                                          </div>
        ...
        {activeCategory !== 'ocean_cleaning' && (
          <div ref={largeFilterPanelRef} className="hidden md:block -mx-4 sm:-mx-6 md:-mx-8 px-4 sm:px-6 md:px-8">
            <AnimatePresence>
              {isFilterExpanded && (
              <motion.div
                initial={(isInitialMount.current || isCategoryChanging.current) ? false : { height: 0, opacity: 0 }}
                                  ...
                                  {selectedLocations.length > 0 && (
                                    <>
                                      <button
                                        onClick={() => setIncludeCommon(prev => !prev)}
                                        className={cn(
                                          "px-3 py-1.5 rounded-xl text-[10.5px] font-black cursor-pointer transition-all flex items-center gap-1.5 border shadow-xs",
                                          !includeCommon
                                            ? "bg-blue-600 border-blue-500 text-white dark:bg-blue-600 dark:border-blue-500 hover:bg-blue-700 shadow-md ring-2 ring-blue-500/30"
                                            : "bg-stone-100 border-stone-300 text-stone-600 hover:bg-stone-200 dark:bg-stone-850 dark:border-stone-750 dark:text-stone-300 dark:hover:bg-stone-800"
                                        )}
                                      >
                                        <span className={cn(
        ...
        {filteredItems.length > 0 ? (
          <>
            <EncyclopediaItemList
              ...
              masterIds={masterIds}
              ratings={ratings}
              toggleCompletion={memoizedToggleCompletion}
              toggleMaster={memoizedToggleMaster}
              handleRate={memoizedHandleRate}
              favorites={favorites}
            ...
            {filteredItems.length > visibleLimit && (
              <div className="flex justify-center mt-8 mb-12">
                <button
                  type="button"
                  onClick={() => setVisibleLimit(prev => Math.min(prev + 24, filteredItems.length))}
                  className="px-8 py-3 bg-neutral-900 hover:bg-neutral-800 dark:bg-stone-100 dark:hover:bg-stone-200 text-white dark:text-stone-900 rounded-2xl font-bold text-sm shadow-md cursor-pointer transition-all hover:scale-[1.02] active:scale-[0.98] inline-flex items-center gap-2"
                >
                  <span>도감 더 보기 ({visibleLimit} / {filteredItems.length})</span>
          ...
          <motion.div 
            ...
            exit={{ opacity: 0, y: 10 }}
            className="empty-view-wrapper bg-white dark:bg-stone-900 rounded-3xl border border-neutral-100 dark:border-stone-800 shadow-sm max-w-[1240px] mx-auto w-[calc(100%-1rem)] sm:w-[calc(100%-2rem)] lg:w-[calc(100%-3rem)] mb-10"
          >
            <div className="empty-view-icon-box !rounded-full mb-6">
              <SearchX className="h-8 w-8 text-neutral-300 dark:text-stone-550" />
            </div>
      ...
      <IngredientUsageModal
        ...
        rawIngredient={selectedIngredientForModal}
        ...
        onSelectDish={(dishName) => {
          dispatch({ type: 'SET_SEARCH_QUERY', payload: dishName });
        }}
      />
    </>
  );

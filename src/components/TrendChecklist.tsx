...
export const TrendChecklist: React.FC = () => {
  ...
  useEffect(() => {
    try {
      const seen = localStorage.getItem('trend_checklist_save_guide_seen');
      if (!seen) {
  ...
  const handleConfirmGuideModal = () => {
    try {
      localStorage.setItem('trend_checklist_save_guide_seen', 'true');
    } catch {
      // Ignore storage errors
    }
  ...
  return (
    <div className="max-w-[1240px] mx-auto w-full px-4 md:px-6 py-6 font-sans select-none animate-in fade-in duration-300 relative">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1 text-stone-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors cursor-pointer rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-950/50"
                  title="저장 기능 안내 보기"
                >
      ...
      <AnimatePresence>
        ...
        {/* New Feature Announcement Guide Modal */}
        {showGuideModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 font-sans">
            <motion.div
              initial={{ opacity: 0 }}
            ...
            <motion.div
              ...
              exit={{ scale: 0.9, y: 20, opacity: 0 }}
              transition={{ type: "spring", duration: 0.4, bounce: 0.35 }}
              className="relative w-full max-w-md bg-white dark:bg-stone-900 rounded-3xl p-6 shadow-2xl border border-stone-200 dark:border-stone-700 text-left z-10 overflow-hidden space-y-4"
            >
              {/* Top Accent Gradient */}
              <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-indigo-500 via-purple-500 to-violet-600" />

              <div className="flex items-center justify-between pt-1">
                <span className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 rounded-lg text-[11px] font-black border border-indigo-200/80 dark:border-indigo-800/80 flex items-center gap-1">
                  <Sparkles className="h-3.5 w-3.5 text-indigo-500 fill-indigo-500" />
                  신규 기능 안내
                </span>
                <button
                  type="button"
                  onClick={handleConfirmGuideModal}
                  className="text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 p-1.5 rounded-xl cursor-pointer"
                >
                  <X className="h-4 w-4" />
                </button>
              ...
              <div className="space-y-1">
                <h3 className="text-base font-black text-stone-900 dark:text-stone-100 flex items-center gap-1.5">
                  수집 현황을 지인들에게 공유해보세요! 💌
                </h3>
                <p className="text-xs text-stone-500 dark:text-stone-400 font-bold leading-relaxed">
                  체크리스트가 한눈에 보이는 이미지로 저장되어 간편하게 공유할 수 있어요.
                </p>
              </div>

              {/* Highlight Mini UI Preview Box (Styled after Trend Shop Progress Card) */}
              <div className="p-4 bg-gradient-to-br from-indigo-50/90 via-purple-50/40 to-stone-50 dark:from-stone-850 dark:via-indigo-950/30 dark:to-stone-850 rounded-2xl border border-indigo-100/90 dark:border-indigo-900/60 shadow-inner space-y-3 relative overflow-hidden">
                {/* Mini Progress Card Simulation */}
                <div className="bg-white dark:bg-stone-900 p-3.5 rounded-xl border border-stone-200/80 dark:border-stone-700 shadow-xs space-y-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="p-1.5 bg-indigo-50 dark:bg-indigo-950/70 text-indigo-600 dark:text-indigo-400 rounded-lg">
                        <Shirt className="h-4 w-4" />
                      </span>
                      <div>
                        <span className="text-xs font-black text-stone-900 dark:text-stone-100 block leading-tight">트렌드상점 수집율</span>
                        <span className="text-[9px] text-stone-400 dark:text-stone-500 font-bold block">TREND SHOP PROGRESS</span>
                      </div>
                    </div>

                    {/* Animated Highlight Button Badge */}
                    <div className="flex items-center gap-1 px-2.5 py-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-violet-600 text-white rounded-lg text-[10px] font-extrabold shadow-sm animate-pulse ring-2 ring-indigo-400/40 ring-offset-1 dark:ring-offset-stone-900">
                      <Download className="h-3 w-3 shrink-0" />
                      <span className="whitespace-nowrap">체크리스트 저장</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 pt-0.5">
                    <div className="flex justify-between text-[10px] font-black">
                      ...
                      <span className="text-stone-400 dark:text-stone-500 font-bold">(31/36)</span>
                    </div>
                    <div className="w-full h-2 bg-stone-100 dark:bg-stone-800 rounded-full overflow-hidden border border-stone-200/50 dark:border-stone-700/50">
                      <div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-violet-600 rounded-full w-[85%]" />
                    </div>
                  </div>
                </div>

                {/* Quick Feature Explanation Points */}
                <ul className="text-xs text-stone-600 dark:text-stone-300 font-bold space-y-1.5 pt-0.5 leading-relaxed">
                  <li className="flex items-start gap-1.5">
                    <span className="text-indigo-500 dark:text-indigo-400 shrink-0 mt-0.5">•</span>
                    <span><strong className="text-indigo-600 dark:text-indigo-300 font-black">[체크리스트 저장]</strong> 버튼을 누르면 수집 목록이 이미지로 즉시 저장됩니다.</span>
                  </li>
                  <li className="flex items-start gap-1.5">
                    <span className="text-indigo-500 dark:text-indigo-400 shrink-0 mt-0.5">•</span>
                    <span>전체 리스트 또는 원하는 특정 카테고리만 선택해서 자유롭게 공유해 보세요!</span>
                  </li>
                </ul>
              </div>
              ...
              <div className="pt-2">
                <button
                  type="button"
                  onClick={handleConfirmGuideModal}
                  className="w-full py-3 rounded-2xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-xs font-black shadow-md shadow-indigo-500/20 hover:from-indigo-600 hover:to-violet-700 cursor-pointer active:scale-95 transition-all text-center"
                >
                  확인했어요
                </button>
              </div>

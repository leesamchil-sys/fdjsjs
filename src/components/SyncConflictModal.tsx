...
export const SyncConflictModal: React.FC<SyncConflictModalProps> = ({
  ...
  const [showDiffDetails, setShowDiffDetails] = React.useState(false);

  const translateField = (field: string) => {
    switch (field) {
      case 'birds': return '새 도감 기록';
      case 'insects': return '곤충 도감 기록';
      case 'fish': return '물고기 도감 기록';
      case 'food': return '음식/요리 도감 기록';
      case 'gardening': return '가드닝 도감 기록';
      case 'pets': return '반려동물(펫) 목록';
      case 'ratings': return '아이템 별점/평가';
      case 'weeklyWeather': return '주간 날씨';
      case 'detailedWeather': return '상세 날씨 정보';
      case 'slots': return '농장 작물 알림 슬롯';
      case 'flowerColors': return '꽃 색상 컬렉션';
      ...
      {syncConflict && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-md bg-white rounded-[32px] shadow-2xl border border-stone-100 overflow-hidden z-10 flex flex-col max-h-[90vh]"
          >
              ...
              {!showOverwriteConfirm ? (
                <>
                  <div className="flex flex-col items-center text-center space-y-3">
                    ...
                    {syncConflict.diffFields && syncConflict.diffFields.length > 0 && (
                      <div className="w-full pt-1">
                        <button
                          onClick={() => setShowDiffDetails(!showDiffDetails)}
                          className="text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100/60 px-3 py-1.5 rounded-xl transition-all inline-flex items-center gap-1.5 cursor-pointer mx-auto"
                        >
                          <span>🔍 충돌 발생 항목(차이점) 확인하기</span>
                          <span className="text-[10px] bg-indigo-200 text-indigo-800 px-1.5 py-0.5 rounded-full font-black">
                            {syncConflict.diffFields.length}개
                          </span>
                        </button>
                        ...
                        {showDiffDetails && (
                          <motion.div
                            ...
                            className="mt-3 p-3 bg-stone-50 border border-stone-200 rounded-2xl text-left text-xs text-stone-600 space-y-1"
                          >
                            <div className="font-bold text-slate-700 mb-1">📋 서버와 로컬 데이터 간 차이가 발견된 항목:</div>
                            <ul className="list-disc pl-4 space-y-0.5 text-[11px] text-stone-500 font-medium">
                              {syncConflict.diffFields.map((field) => (

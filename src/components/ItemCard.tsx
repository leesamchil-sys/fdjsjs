...
const SEASON_BADGE_STYLE: Record<string, { bg: string; text: string; border: string }> = {
  event_1: {
...
export function ItemCard({ 
  ...
  const badgeStyle = item.seasonId ? (SEASON_BADGE_STYLE[item.seasonId] || {
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'price'>(hidePrices || !hasPrice ? 'info' : 'price');

  useEffect(() => {
    setActiveTab(hidePrices || !hasPrice ? 'info' : 'price');
  }, [hidePrices, hasPrice]);
  ...
  const matchesWeatherInternal = (itemWeather: string, contextWeather: GameWeather) => {
    ...
    const actualContextWeather = contextWeather === 'Heatwave' ? 'Clear' : contextWeather;
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "group relative flex flex-col w-full overflow-hidden rounded-[32px] border transition-all duration-500",
      ...
      <div className="flex flex-col h-full pt-4 sm:pt-5 px-3 sm:px-5 pb-5 sm:pb-6.5">
          ...
          <div className="flex items-center gap-1 shrink-0 flex-nowrap">
            ...
            {type === 'cooking' ? (
              ...
              <span className={cn(
                ...
                {item.weather ? formatWeatherValue(item.weather) : '날씨무관'}
              </span>
            )}
        ...
        <div className="py-2.5 sm:py-3 relative flex items-center justify-between gap-3">
          ...
          <div className={cn(
            ...
            {(() => {
              const imgUrl = item.imageUrl || getExistingImagePath(type, item.name);
        ...
        <div className={cn(
          ...
          {type === 'cooking' ? (
                    ...
                    {item.ingredients ? (
                      item.ingredients.map((ing: string, idx: number) => (
                        <span 
                          key={`ing-${item.id}-${idx}`}
                          className={cn(
                            "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10.5px] font-extrabold tracking-tight transition-all border shadow-2xs",
                            useRainbowStyle
                              ? "bg-white dark:bg-stone-900/60 border-indigo-200 dark:border-indigo-800/80 text-indigo-950 dark:text-indigo-200"
                              : "bg-stone-50 dark:bg-stone-850 border-stone-200/50 dark:border-stone-800 text-stone-600 dark:text-stone-400 hover:border-amber-300 dark:hover:border-amber-700"
                          )}
                        >
            ...
            <div className="flex flex-col gap-1.5">
              {/* TABS (Only show if item has price) */}
              {hasPrice && (
                <div className="flex items-center gap-1 px-1">
                  <button
                    onClick={() => setActiveTab('info')}
                    ...
                    <div className={cn("transition-opacity", isRecommend && recommendTarget === 'fivestar' && !isGenMatched ? "opacity-35 grayscale" : "opacity-100")}>
                      ...
                      {type === 'ocean_cleaning' ? (
                        ...
                        <div className="flex flex-col gap-1 w-full text-[10.5px] font-extrabold">
                          ...
                          <div className="flex items-center gap-1.5 min-w-0 text-stone-600 dark:text-stone-300">
                            <MapPin className="h-3.5 w-3.5 shrink-0 text-stone-400 dark:text-stone-500" />
                            <span className="truncate">{item.locations ? item.locations.join(', ') : '정보 없음'}</span>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <div className="flex items-center gap-0.5 -ml-1">
              {[1, 2, 3, 4, 5].map((star) => (
                <button
                  ...
                  onClick={(e) => {
                     e.stopPropagation();
                     onRate(item.name, rating === star ? 0 : star);
                     setHoverRating(0);
                  }}

...
import { ChevronLeft, ChevronRight, Home, Bird as BirdIcon, Bug, Fish as FishIcon, Soup, Flower, Sprout, Heart, Settings, Ticket, Waves, Shirt } from 'lucide-react';
...
export const DesktopSidebar: React.FC<DesktopSidebarProps> = React.memo(({
  ...
  const menuItems = [
    ...
    { id: 'trend_checklist', label: '트렌드상점 체크리스트', icon: Shirt, total: null, completed: null, badge: null }
  ].filter(item => {
            ...
            {menuItems.map(item => (
              item.id === 'divider' || item.id === 'divider_home' ? (
                ...
                <button
                  ...
                  className={cn(
                    "w-full flex items-center py-2 rounded-lg text-sm font-medium transition-all group cursor-pointer",
                    isDesktopSidebarExpanded ? "px-3 justify-between" : "px-0 justify-center",
                    activeCategory === item.id 
                      ? "bg-slate-900 text-white dark:bg-stone-100 dark:text-stone-900 font-extrabold shadow-sm" 

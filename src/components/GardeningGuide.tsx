...
import { Search, Star, Medal, CheckSquare, Clock, RefreshCcw, X, ChevronDown, Eye, EyeOff, Calendar, Sparkles, Settings, ChevronRight, Compass, Heart } from 'lucide-react';
import { cn, safeJsonParse } from '../lib/utils';
import { GARDENING_ITEMS, FLOWER_IMAGE_MAPPING, CROP_IMAGE_MAPPING } from '../data/gardening';
...
function GardeningCard({
  ...
  const imgUrl = flowerPrefix 
    ? `/images/gardening/${item.name}/${flowerPrefix}_1.png`
    : cropFilename
      ? `/images/crops/${cropFilename}.png`
      : `/images/${imageFolder}/${item.name}.png`;
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col w-full overflow-hidden rounded-[32px] border transition-all duration-500 select-none cursor-default shadow-sm",
    ...
    >
          ...
          <div className="flex items-center justify-between border-t border-neutral-100 dark:border-stone-800 pt-2 border-dashed flex-wrap gap-1.5">
            <div className="flex items-center gap-0.5 -ml-1">
              {[1, 2, 3, 4, 5].map((starIdx) => {
                const active = starIdx <= currentRating;
                return (
                  <button
                    key={starIdx}
                    type="button"
                    onClick={(e) => {
                      e.stopPropagation();
                      handleRate(item.id, item.name, starIdx);
                    }}
      ...
      {isMaster && (
        <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none opacity-[0.25]">
           <img src="/images/medal.png" alt="Master Badge" className="w-28 h-28 sm:w-32 sm:h-32 object-contain" />
        </div>
      )}
    </motion.div>
  );
  ...
  return (
    <div id="gardening-guide-layout" className="w-full mx-auto text-stone-800 dark:text-stone-100">
      ...
      <div 
        ...
        className="sticky top-[var(--sticky-top-mobile,56px)] lg:top-[var(--sticky-top-desktop,0px)] z-[50] -mx-4 sm:-mx-6 md:-mx-8 bg-neutral-50/95 dark:bg-stone-950/95 backdrop-blur-md px-4 sm:px-6 md:px-8 py-2 md:py-3 border-b border-stone-200/35 dark:border-stone-900/40 shadow-xs mb-6 font-scale-lock"
      >
          ...
          <div className="rounded-2xl sm:rounded-[32px] border border-stone-200/40 dark:border-stone-800/50 bg-white dark:bg-stone-900 shadow-xl shadow-neutral-200/30 dark:shadow-none overflow-visible">
            <div className="p-1.5 sm:p-4 flex flex-col gap-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3 w-full">
                <div className="relative flex-1 group overflow-hidden h-[36px] sm:h-[42px]">
                  <Search className={cn(
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      favoritesTab === 'favorites'
                        ...
                        : "bg-white dark:bg-stone-900 text-stone-550 hover:text-stone-700 dark:text-stone-400 dark:hover:text-stone-200 border-stone-200/60 dark:border-stone-800 hover:border-stone-300 dark:hover:border-stone-700 shadow-2xs"
                    )}
      ...
      <div className="px-1 sm:px-2 pb-8">
        ...
        <div className="flex items-center justify-between mb-4 pb-1 border-b border-stone-200/65 dark:border-stone-850 px-1 max-w-[1240px] mx-auto w-full">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-neutral-900 dark:text-stone-100">검색 결과</span>
        ...
        <AnimatePresence mode="popLayout" initial={false}>
          {filteredItems.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-xs font-bold text-stone-500 dark:text-stone-400 mt-1">도감 검색 결과가 없습니다.</h3>
            </motion.div>
          ) : (
            <motion.div

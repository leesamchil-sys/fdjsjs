...
function GardeningCard({
  ...
  const filteredVariants = variants.filter(v => !v.toLowerCase().includes('thu'));
  const totalColorCount = filteredVariants.length;
  const collectedColorCount = filteredVariants.filter(v => flowerColors[v]).length;
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col w-full overflow-hidden rounded-[32px] border transition-all duration-200 select-none cursor-default shadow-sm",
        "hover:border-stone-400 dark:hover:border-stone-500 hover:shadow-md",
        "border-stone-200/60 dark:border-stone-800 bg-white dark:bg-stone-900 text-stone-900 dark:text-stone-100"
      )}
  ...
  return (
    <div id="gardening-guide-layout" className="w-full mx-auto text-stone-800 dark:text-stone-100">
      ...
      <div 
        ...
        className="sticky top-[var(--sticky-top-mobile)] lg:top-[var(--sticky-top-desktop)] z-[50] -mx-4 sm:-mx-6 md:-mx-8 bg-neutral-50/95 dark:bg-stone-950/95 backdrop-blur-md px-4 sm:px-6 md:px-8 py-2 md:py-3 border-b border-stone-200/35 dark:border-stone-900/40 shadow-xs mb-6 font-scale-lock"
      >

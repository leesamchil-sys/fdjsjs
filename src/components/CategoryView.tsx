...
const CropTimer = React.lazy(() => import('./CropTimer'));
const GardeningGuide = React.lazy(() => import('./GardeningGuide'));
const PetFoodFinder = React.lazy(() => import('./PetFoodFinder'));
const TrendChecklist = React.lazy(() => import('./TrendChecklist').then(m => ({ default: m.TrendChecklist })));
const EncyclopediaSection = React.lazy(() => import('./EncyclopediaSection'));
const PrivacyPolicyInner = React.lazy(() => import('./PrivacyPolicyInner'));
const TermsOfServiceInner = React.lazy(() => import('./TermsOfServiceInner'));
const CouponSection = React.lazy(() => import('./CouponSection'));
...
const OptimizedHiddenView = React.memo(({ isActive, deps, render }: { isActive: boolean; deps?: any[]; render: () => React.ReactNode }) => {
  return (
    <div style={isActive ? {} : { opacity: 0, height: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {render()}
    </div>
  );
}, (prevProps, nextProps) => {
  // If active state changed, we must re-render to update the wrapper div className
  if (prevProps.isActive !== nextProps.isActive) return false;
  
  // If dependencies changed (e.g., background sync state), we must re-render even if hidden
  ...
  // If it's currently hidden and stays hidden (and no deps changed), skip re-rendering entirely!
  if (!nextProps.isActive) return true;
  
  // If it's active and stays active, we must re-render to reflect data changes.
  return false; 
});
  ...
  return (
    <Suspense fallback={
      ...
      <OptimizedHiddenView 
        ...
        deps={[props.completedBirdIds, props.completedInsectIds, props.completedFishIds, props.completedFoodIds, props.masterBirdIds, props.masterInsectIds, props.masterFishIds, props.masterFoodIds, props.masterGardeningIds, props.ratings, props.completedFlowerIds, props.completedCropIds, props.pets, props.activeCouponsCount, props.completedOceanCleaningIds, props.masterOceanCleaningIds, props.user?.uid]}
        render={() => (
          <HomeDashboard 
            user={props.user}
      ...
      <OptimizedHiddenView 
        ...
        deps={[props.isInitialSyncDone, props.cropPresets, props.user?.uid]}
        render={() => (
          <CropTimer 
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'gardening'}
        deps={[props.completedGardeningIds, props.masterGardeningIds, props.ratings, props.activeSeasonIds, props.flowerColorCollections]}
        render={() => (
          <GardeningGuide 
      ...
      <OptimizedHiddenView 
        isActive={activeCategory === 'petfood'}
        deps={[props.pets, props.activeSeasonIds, props.user?.uid]}
        render={() => (
          <PetFoodFinder 
            pets={props.pets} 
            setPets={(newPets) => {
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'trend_checklist'}
        render={() => (
          <TrendChecklist />
        )}
      />
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'coupons'}
        deps={[props.user?.uid, props.allowedUids]}
        render={() => (
          <CouponSection 
            user={props.user} 
            allowedUids={props.allowedUids} 
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'birds'}
        deps={[props.birds, props.completedBirdIds, props.masterBirdIds, props.ratings, props.currentTime, props.currentGameWeather, props.activeSeasonIds]}
        render={() => (
          <EncyclopediaSection
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'insects'}
        deps={[props.insects, props.completedInsectIds, props.masterInsectIds, props.ratings, props.currentTime, props.currentGameWeather, props.activeSeasonIds]}
        render={() => (
          <EncyclopediaSection
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'fishing'}
        deps={[props.fish, props.completedFishIds, props.masterFishIds, props.ratings, props.currentTime, props.currentGameWeather, props.activeSeasonIds]}
        render={() => (
          <EncyclopediaSection
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'cooking'}
        deps={[props.cooking, props.completedFoodIds, props.masterFoodIds, props.ratings, props.currentTime, props.currentGameWeather, props.activeSeasonIds]}
        render={() => (
          <EncyclopediaSection
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'ocean_cleaning'}
        deps={[props.oceanCleaning, props.completedOceanCleaningIds, props.masterOceanCleaningIds, props.ratings, props.currentTime, props.currentGameWeather, props.activeSeasonIds]}
        render={() => (
          <EncyclopediaSection
      ...
      <OptimizedHiddenView
        isActive={activeCategory === 'privacy'}
        render={() => <PrivacyPolicyInner />}
      />

      <OptimizedHiddenView
        isActive={activeCategory === 'terms'}
        render={() => <TermsOfServiceInner />}
      />
    </Suspense>
  );
});

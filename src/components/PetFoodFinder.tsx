...
import PetReorderModal from './PetReorderModal';
import ImageEditModal from './ImageEditModal';
import { getPetImage, savePetImage, deletePetImage } from '../utils/indexedDB';
...
export default function PetFoodFinder({ pets, setPets, activeSeasonIds = [], debouncedSyncAllData, markCollectionsModified }: PetFoodFinderProps) {
  ...
  // --- Pet Profile Image Upload States and Effect ---
  const [petImageUrls, setPetImageUrls] = useState<Record<string, string>>({});
  const [uploadTargetPet, setUploadTargetPet] = useState<Pet | null>(null);
  const [activeMenuPetId, setActiveMenuPetId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const loadImages = async () => {
      const petsWithImages = pets.filter(p => p.hasCustomImage);
      for (const pet of petsWithImages) {
        try {
          const blob = await getPetImage(pet.id);
          if (blob && isMounted) {
            const url = URL.createObjectURL(blob);
            newUrls[pet.id] = url;
          }
        } catch (err) {
          console.error(`Failed to load profile image for ${pet.id}:`, err);
        }
      }

      if (isMounted) {
        setPetImageUrls(prev => {
          // Revoke all existing URLs to prevent leaks
  ...
  // Handle clicking outside to close thumbnail menus
  useEffect(() => {
  ...
  const handleSaveProfileImage = async (compressedBlob: Blob) => {
    if (!uploadTargetPet) return;
    
    // Store target info locally to use after state is cleared
    const targetPetName = uploadTargetPet.name;
    
    try {
      await savePetImage(uploadTargetPet.id, compressedBlob);

      setPets(prev => prev.map(p => {
        if (p.id === uploadTargetPet.id) {
          return { ...p, hasCustomImage: true };
        }
        return p;
      }));

      // Trigger persistence and cloud sync metadata
      markCollectionsModified?.();
      debouncedSyncAllData?.();

      const url = URL.createObjectURL(compressedBlob);
      setPetImageUrls(prev => {
        if (prev[uploadTargetPet.id]) {
          URL.revokeObjectURL(prev[uploadTargetPet.id]);
        }
        return { ...prev, [uploadTargetPet.id]: url };
      });
      
      // Close popup first
      setUploadTargetPet(null);
      // Then show toast message
      setTimeout(() => setToastMessage(`📸 '${targetPetName}'의 프로필 사진이 설정되었습니다.`), 50);

    } catch (err) {
      console.error(err);
      setCustomAlert('프로필 사진을 저장하는 동안 오류가 발생했습니다.');
      setUploadTargetPet(null);
    }
  };

  const handleDeleteProfileImage = async (petId: string) => {
    try {
      await deletePetImage(petId);

      setPets(prev => prev.map(p => {
        if (p.id === petId) {
          const { hasCustomImage, ...rest } = p;
          return rest;
        }
        return p;
      }));
      ...
      setPetImageUrls(prev => {
        if (prev[petId]) {
          URL.revokeObjectURL(prev[petId]);
        }
        const updated = { ...prev };
      ...
      const petName = pets.find(p => p.id === petId)?.name || '반려동물';
      setToastMessage(`🗑️ '${petName}'의 프로필 사진이 삭제되었습니다.`);
    } catch (err) {
      console.error(err);
      setCustomAlert('프로필 사진을 삭제하는 동안 오류가 발생했습니다.');
    }
  };
  ...
  const [petImageToDelete, setPetImageToDelete] = useState<{ id: string; name: string } | null>(null);
  ...
  useBackDismiss(showReorderModal, () => setShowReorderModal(false), 'petFoodReorderModal');
  ...
  useBackDismiss(!!petImageToDelete, () => setPetImageToDelete(null), 'petFoodImageDeleteConfirm');
  ...
  const handleSavePetReorder = (newOrderedPets: Pet[]) => {
    setPets(newOrderedPets);
    try {
      localStorage.setItem('pigtown_pets', JSON.stringify(newOrderedPets));
    } catch (e) {
      console.error('Failed to update pigtown_pets in localStorage:', e);
    }
    markCollectionsModified?.();
    debouncedSyncAllData?.(0);
    setToastMessage('반려동물 순서가 성공적으로 변경되었습니다.');
  };
  ...
  const displayActivePets = activePets.filter(p => petTypeFilter === 'all' || p.type === petTypeFilter);
  const displayHotelPets = hotelPets.filter(p => petTypeFilter === 'all' || p.type === petTypeFilter);
  ...
  useEffect(() => {
    if (petTypeFilter === 'all') return;
    const filteredActive = activePets.filter(p => p.type === petTypeFilter);
    if (filteredActive.length > 0 && activePet && activePet.type !== petTypeFilter) {
      setActivePetId(filteredActive[0].id);
    }
  }, [petTypeFilter]);
  ...
  const getFilteredItems = () => {
    ...
    let filtered = sanitizedList.filter(item => {
      ...
      const nameMatch = item.name.replace(/\s+/g, '').toLowerCase().includes(normalizedQuery);
      
      let metaMatch = false;
      if ('ingredients' in item && item.ingredients) {
        metaMatch = item.ingredients.some(ing => ing.replace(/\s+/g, '').toLowerCase().includes(normalizedQuery));
      } else if ('locations' in item && item.locations) {
        metaMatch = item.locations.some(loc => loc.replace(/\s+/g, '').toLowerCase().includes(normalizedQuery));
      }
  ...
  return (
    <div ref={rootRef} className="w-full space-y-6 max-w-[1240px] mx-auto select-none px-1 py-1 overflow-visible">
      ...
      <div className="w-full bg-white dark:bg-stone-900 rounded-2xl p-4 sm:p-5 border border-stone-200/50 dark:border-stone-850 shadow-sm space-y-4">
        ...
        <div className="bg-stone-50/20 dark:bg-stone-950/10 p-2 rounded-xl">
          {pets.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {/* Pet Category Tabs & Filter Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                {/* Species Filter & Reorder Button */}
                <div className="flex items-center gap-1.5 w-full sm:w-auto">
                  <div className="flex-1 sm:flex-initial flex items-center gap-1 bg-stone-100/90 dark:bg-stone-900/70 p-1 rounded-xl border border-stone-200/60 dark:border-stone-800">
                    <button
                      type="button"
                      onClick={() => setPetTypeFilter('all')}
                      className={cn(
                        "flex-1 sm:flex-initial px-3 py-1.5 rounded-lg text-xs font-extrabold transition-all cursor-pointer flex items-center justify-center gap-1",
                        petTypeFilter === 'all'
                          ? "bg-white dark:bg-stone-800 text-stone-850 dark:text-stone-100 shadow-xs"
                          : "text-stone-500 hover:text-stone-800 dark:text-stone-400 dark:hover:text-stone-200"
                      )}
                    ...
                    <button
                      ...
                      className={cn(
                        "flex-1 sm:flex-initial px-3 py-1.5 rounded-lg text-xs font-extrabold transition-all cursor-pointer flex items-center justify-center gap-1",
                        petTypeFilter === 'dog'
                          ? "bg-amber-500 text-white dark:bg-amber-600 shadow-xs font-black"
                          : "text-stone-500 hover:text-amber-600 dark:text-stone-400 dark:hover:text-amber-400"
                      )}
                    >
                    ...
                    <button
                      ...
                      className={cn(
                        "flex-1 sm:flex-initial px-3 py-1.5 rounded-lg text-xs font-extrabold transition-all cursor-pointer flex items-center justify-center gap-1",
                        petTypeFilter === 'cat'
                          ? "bg-sky-500 text-white dark:bg-sky-600 shadow-xs font-black"
                          : "text-stone-500 hover:text-sky-600 dark:text-stone-400 dark:hover:text-sky-400"
                      )}
                    >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl bg-stone-100 hover:bg-stone-200/80 dark:bg-stone-800 dark:hover:bg-stone-700 text-stone-750 dark:text-stone-200 border border-stone-200/80 dark:border-stone-700 transition-all cursor-pointer flex items-center justify-center shrink-0 active:scale-95 shadow-3xs"
                    title="반려동물 순서 변경"
                    aria-label="반려동물 순서 변경"
                  >
                    <ArrowUpDown className="h-4 w-4 text-amber-500 dark:text-amber-400" />
                  </button>
                </div>
              </div>
              ...
              {activePetTab === 'active' && (
                <>
                  ...
                  ) : displayActivePets.length === 0 ? (
                    <div className="py-6 text-center text-xs text-stone-400 dark:text-stone-500 font-bold bg-white dark:bg-stone-950/20 rounded-xl border border-dashed border-stone-200 dark:border-stone-800">
                      🐾 선택한 필터({petTypeFilter === 'dog' ? '강아지' : '고양이'})에 해당하는 마이펫이 없어요.
                    </div>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      <div className="max-h-[220px] min-h-[140px] overflow-y-auto pb-14 pr-1 custom-scrollbar">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
                          {displayActivePets.map((p, idx) => {
                            ...
                            const isMenuOpen = activeMenuPetId === p.id;
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  ...
                                  isMenuOpen && "z-30 relative ring-2 ring-amber-400 dark:ring-amber-500",
                                  isActive
                                    ? isDog
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                  <div className="relative shrink-0 select-none">
                                    <div 
                                      className={cn(
                                        "rounded-xl transition-colors flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 overflow-hidden bg-stone-100 dark:bg-stone-800 cursor-pointer shrink-0",
                                        !petImageUrls[p.id] && (isDog ? "bg-amber-100 dark:bg-amber-955/40 text-amber-600" : "bg-sky-100 dark:bg-sky-955/40 text-sky-600")
                                      )}
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        if (p.hasCustomImage) {
                                          setActiveMenuPetId(activeMenuPetId === p.id ? null : p.id);
                                        } else {
                                          setUploadTargetPet(p);
                                        }
                                      }}
                                    >
                                      {petImageUrls[p.id] ? (
                                        <img
                                          src={petImageUrls[p.id]}
                                          alt={p.name}
                                    ...
                                    <button
                                      ...
                                      className="absolute -bottom-1 -right-1 p-0.5 bg-amber-500 hover:bg-amber-600 text-white rounded-full shadow-md hover:scale-110 transition-transform cursor-pointer border border-white dark:border-stone-900"
                                      title="프로필 편집"
                                    >
                                    ...
                                    {/* Small Dropdown for Image Change/Delete */}
                                    {isMenuOpen && (
                                      <div className={cn(
                                        "absolute left-0 z-40 w-28 bg-white dark:bg-stone-900 rounded-lg shadow-2xl border border-stone-200 dark:border-stone-800 py-1 text-[11px] font-bold animate-in fade-in zoom-in duration-100",
                                        idx >= displayActivePets.length - 2 && displayActivePets.length > 2
                                          ? "bottom-full mb-1.5"
                                          : "top-full mt-1.5"
                                      )}>
                                        <button
                                          ...
                                          className="w-full px-2.5 py-1.5 text-left text-stone-700 dark:text-stone-200 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors flex items-center gap-1.5"
                                        >
                                          <span>🖼️ 이미지 변경</span>
                                        </button>
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setActiveMenuPetId(null);
                                            setPetImageToDelete({ id: p.id, name: p.name });
                                          }}
                                          className="w-full px-2.5 py-1.5 text-left text-rose-600 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors flex items-center gap-1.5 border-t border-stone-100 dark:border-stone-800"
                                        >
                                          <span>❌ 이미지 삭제</span>
                                        </button>
                                      </div>
                                  ...
                                  <div className="min-w-0 flex-1 pr-1">
                                    <p className="text-[11.5px] sm:text-xs font-extra-bold text-slate-800 dark:text-stone-100 truncate" title={p.name}>
                                      {p.name}
                                    </p>
              ...
              {activePetTab === 'hotel' && (
                <>
                  ...
                  ) : displayHotelPets.length === 0 ? (
                    <div className="py-8 text-center text-xs text-stone-400 dark:text-stone-500 font-bold bg-white dark:bg-stone-950/20 rounded-xl border border-dashed border-stone-200 dark:border-stone-800 space-y-2">
                      <div className="text-xl">🏡</div>
                      <p>선택한 필터({petTypeFilter === 'dog' ? '강아지' : '고양이'})에 해당하는 펫 호텔 반려동물이 없어요.</p>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      <div className="max-h-[220px] min-h-[140px] overflow-y-auto pb-14 pr-1 custom-scrollbar">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
                          {displayHotelPets.map((p, idx) => {
                            const isDog = p.type === 'dog';
                            const countLikes = Object.values(p.preferences || {}).filter(v => v === 'like').length;
                            const countDislikes = Object.values(p.preferences || {}).filter(v => v === 'dislike').length;
                            const isMenuOpen = activeMenuPetId === p.id;
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  "group relative flex items-center justify-between p-2.5 sm:p-3 rounded-xl transition-all cursor-pointer select-none border bg-stone-100/40 dark:bg-stone-900/30 border-stone-200/60 dark:border-stone-800/60 opacity-70 hover:opacity-100",
                                  isMenuOpen && "z-30 relative opacity-100 ring-2 ring-amber-400 dark:ring-amber-500"
                                )}
                              >
                                <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                                  <div className="relative shrink-0 select-none">
                                    <div 
                                      className={cn(
                                        "rounded-xl transition-colors flex items-center justify-center h-9 w-9 sm:h-10 sm:w-10 overflow-hidden bg-stone-100/50 dark:bg-stone-850/30 opacity-70 cursor-pointer shrink-0",
                                        !petImageUrls[p.id] && (isDog ? "bg-amber-100/50 dark:bg-amber-955/20 text-amber-600/70" : "bg-sky-100/50 dark:bg-sky-955/20 text-sky-600/70")
                                      )}
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        if (p.hasCustomImage) {
                                    ...
                                    {isMenuOpen && (
                                      <div className={cn(
                                        "absolute left-0 z-40 w-28 bg-white dark:bg-stone-900 rounded-lg shadow-2xl border border-stone-200 dark:border-stone-800 py-1 text-[11px] font-bold animate-in fade-in zoom-in duration-100",
                                        idx >= displayHotelPets.length - 2 && displayHotelPets.length > 2
                                          ? "bottom-full mb-1.5"
                                          : "top-full mt-1.5"
                                      )}>
                                        <button
                                        ...
                                        <button
                                          ...
                                          className="w-full px-2.5 py-1.5 text-left text-rose-650 dark:text-rose-400 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors flex items-center gap-1.5 border-t border-stone-100 dark:border-stone-800"
                                        >
                                          <span>❌ 이미지 삭제</span>
                                        </button>
                                      </div>
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="text-[11.5px] sm:text-xs font-extra-bold text-stone-600 dark:text-stone-400 truncate" title={p.name}>
                                      {p.name}
                                    </p>
                                    <div className="flex items-center gap-2 mt-0.5">
      ...
      {activePet && activePetTab === 'active' && (
        <div 
          ref={stickyHeaderRef}
          className="sticky top-[var(--sticky-top-mobile)] lg:top-[var(--sticky-top-desktop)] z-[50] -mx-4 sm:-mx-6 md:-mx-8 px-4 sm:px-6 md:px-8 py-2 md:py-3 bg-neutral-50 dark:bg-stone-950 border-y border-stone-200/60 dark:border-stone-900/80 transition-all font-sans mb-6"
        >
          <div className="max-w-[1240px] mx-auto w-full px-1">
            <div className="bg-white dark:bg-stone-900/60 rounded-2xl sm:rounded-3xl border border-stone-200/50 dark:border-stone-800/50 shadow-xs p-2 sm:p-4 flex flex-col gap-1.5 sm:gap-3">
              ...
              <div className="sm:hidden flex flex-col gap-1.5 w-full">
                ...
                <div className="flex items-center gap-1.5 w-full">
                  <div className="relative shrink-0" ref={mobileDropdownRef}>
                    ...
                    >
                      <div className={cn(
                        "rounded-md shrink-0 transition-colors flex items-center justify-center h-6 w-6 overflow-hidden bg-stone-100 dark:bg-stone-800",
                        !petImageUrls[activePet.id] && (activePet.type === 'dog' 
                          ? "bg-amber-100 dark:bg-amber-955/40 text-amber-600" 
                          : "bg-sky-100 dark:bg-sky-955/40 text-sky-600")
                      )}>
                        {petImageUrls[activePet.id] ? (
                          <img
                            src={petImageUrls[activePet.id]}
                            alt={activePet.name}
                            referrerPolicy="no-referrer"
                      ...
                      <span className="max-w-[150px] truncate">{activePet.name}</span>
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-2 truncate">
                                      <div className={cn(
                                        "rounded-md shrink-0 transition-colors flex items-center justify-center h-5.5 w-5.5 overflow-hidden bg-stone-100 dark:bg-stone-800",
                                        !petImageUrls[p.id] && (p.type === 'dog' 
                                          ? "bg-amber-100 dark:bg-amber-955/40 text-amber-600" 
                                          : "bg-sky-100 dark:bg-sky-955/40 text-sky-600")
                                      )}>
                                        {petImageUrls[p.id] ? (
                  ...
                  <div className="relative flex-1 group overflow-hidden search-bar-container !h-[34px]">
                    <Search className={cn(
                    ...
                    <input
                      ...
                      className="search-bar-input !pl-8.5 !text-xs"
                    />
                    {searchQuery && (
                      <button
                        type="button"
              ...
              <div className="hidden sm:flex flex-col gap-3 w-full">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2.5 border-b border-stone-150/60 dark:border-stone-850/60">
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className={cn(
                      "rounded-xl shrink-0 transition-colors flex items-center justify-center h-10 w-10 overflow-hidden bg-stone-100 dark:bg-stone-800",
                      !petImageUrls[activePet.id] && (activePet.type === 'dog' 
                        ? "bg-amber-100 dark:bg-amber-955/40 text-amber-600" 
                        : "bg-sky-100 dark:bg-sky-955/40 text-sky-600")
                    )}>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative" ref={dropdownRef}>
                        <button
                          type="button"
                          onClick={() => setShowDropdown(prev => !prev)}
                          className="flex items-center gap-1.5 bg-stone-100/70 hover:bg-stone-200/60 dark:bg-stone-800/70 dark:hover:bg-stone-700/60 rounded-lg px-2.5 py-1.5 transition-all cursor-pointer font-black text-xs text-slate-855 dark:text-stone-100 shadow-3xs group select-none min-w-[70px] max-w-[200px] justify-between"
                        >
                          <span className="truncate max-w-[160px] pr-0.5">{activePet.name}</span>
      ...
      {activePet && activePetTab === 'active' && (
        <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 content-start min-h-[calc(100vh-300px)]">
          {filteredItems.map(item => {
            ...
            return (
              <motion.div
                key={item.id}
                className={cn(
                  "group relative overflow-hidden p-3 sm:p-4 flex flex-col justify-between card-layout",
                  pref === 'neutral' && !isRainbow && "base-card-style",
                  pref === 'neutral' && isRainbow && "rainbow-card-style",
                  pref === 'like' && "bg-emerald-50/85 dark:bg-emerald-950/20 border-emerald-300/85 dark:border-emerald-800/40 ring-1 ring-emerald-400/30 hover:border-emerald-400 dark:hover:border-emerald-700",
                  pref === 'dislike' && "bg-rose-50/85 dark:bg-rose-950/20 border-rose-300/85 dark:border-rose-800/40 ring-1 ring-rose-400/30 hover:border-rose-400 dark:hover:border-rose-700"
                )}
              >
                ...
                {itemLevel !== undefined && (() => {
                  ...
                  return (
                    <span className={cn(
                      "card-overlay-badge top-2.5 left-3 sm:text-[11.5px]",
                      levelBadgeStyle
                    )}>
                      Lv.{itemLevel}
                    </span>
                ...
                <button
                  ...
                  className="absolute top-2 right-2 z-20 p-1 text-rose-500/80 hover:text-rose-600 dark:text-rose-400 dark:hover:text-rose-300 transition-all hover:scale-110 active:scale-95 cursor-pointer flex items-center justify-center shrink-0"
                >
                ...
                {seasonEvent && (() => {
                  ...
                  return (
                    <span className={cn(
                      "absolute top-2.5 right-10 z-10 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black border shadow-2xs backdrop-blur-xs whitespace-nowrap",
                      badgeStyle.bg,
                      badgeStyle.text,
                      badgeStyle.border
                    )}>
          ...
          {filteredItems.length === 0 && (
            <div className="col-span-full empty-view-wrapper bg-white/50 dark:bg-stone-900/30 rounded-3xl border border-stone-200 dark:border-stone-800 shadow-sm mt-4">
      ...
      <AnimatePresence>
        ...
        {petImageToDelete && (
          <div key="pet-image-delete-dialog" className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs select-none">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
            ...
            >
              <div className="flex items-center gap-2.5 text-rose-500">
                ...
                <h3 className="text-sm font-black text-stone-850 dark:text-stone-100">프로필 사진 삭제</h3>
              </div>
              <p className="text-xs font-bold text-stone-600 dark:text-stone-300 whitespace-pre-line leading-relaxed text-left">
                ‘{petImageToDelete.name}’ 의 커스텀 프로필 사진을 정말 삭제하시겠습니까?
              </p>
              <div className="flex justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => setPetImageToDelete(null)}
                ...
                <button
                  type="button"
                  onClick={() => {
                    handleDeleteProfileImage(petImageToDelete.id);
                    setPetImageToDelete(null);
                  }}
      ...
      <PetReorderModal
        ...
        pets={pets}
        onSave={handleSavePetReorder}
        petImageUrls={petImageUrls}
      />

      {/* Image Edit Modal */}
      <ImageEditModal
        isOpen={!!uploadTargetPet}
        onClose={() => setUploadTargetPet(null)}
        petName={uploadTargetPet?.name || ''}

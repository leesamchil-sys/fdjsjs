  ...
  return (
    <div ref={rootRef} className="space-y-6 max-w-[1240px] mx-auto select-none px-1 py-1 overflow-visible">
        ...
        <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 content-start min-h-[calc(100vh-300px)]">
          {filteredItems.map(item => {
            ...
            return (
              ...
              >
                {/* Seasonal Badge on Top-Right */}
                {seasonEvent && (() => {
                  const badgeStyle = getSeasonBadgeStyle(seasonEvent.id);
                  return (
                    <span className={cn(
                      "absolute top-2 right-2 z-10 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] sm:text-[10px] font-black border shadow-2xs backdrop-blur-xs",
                      badgeStyle.bg,
                      badgeStyle.text,
                      badgeStyle.border
                    )}>
      ...
      <AnimatePresence>
        ...
        {showHotelGuide && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-xs select-none overflow-y-auto">
            <motion.div

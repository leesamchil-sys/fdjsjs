...
const isNewCoupon = (createdAt?: string) => {
  const normalized = parseCreatedAt(createdAt);
...
export default function CouponSection({ user, allowedUids, isActive }: CouponSectionProps) {
  ...
  const loadCoupons = async () => {
    try {
      ...
      if (isCacheValid) {
        try {
          ...
          if (parsed && Array.isArray(parsed)) {
            console.log("[Cache] CouponSection - Loaded and normalizing coupons from local cache");
      ...
      console.log(`[GET_DOCS] settings_coupons - path: ${couponsRef.path}`);
      ...
      // Save to cache
      try {
        localStorage.setItem('pt_cached_coupons', JSON.stringify(sorted));
        localStorage.setItem('pt_cached_coupons_time', Date.now().toString());
      } catch (e) {
        console.warn("Coupons caching error in CouponSection:", e);
      }
  ...
  // Lazy load only when active and not already loaded or cache is expired
  useEffect(() => {
    if (isActive) {
      const cacheTime = localStorage.getItem('pt_cached_coupons_time');
      const cachedData = localStorage.getItem('pt_cached_coupons');
      ...
      if (isCacheValid && coupons.length > 0) {
        // Already loaded in state and cache is still valid
        setLoading(false);
        return;
      }
      loadCoupons();
  ...
  const handleDelete = async (id?: string) => {
    ...
    try {
      ...
      // Clear cache so that the next reload fetches fresh data, and update active state
      localStorage.removeItem('pt_cached_coupons');
      localStorage.removeItem('pt_cached_coupons_time');
      window.dispatchEvent(new Event('coupons_data_changed'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear cache so that the next loadCoupons or reload fetches fresh data
      localStorage.removeItem('pt_cached_coupons');
      localStorage.removeItem('pt_cached_coupons_time');
      window.dispatchEvent(new Event('coupons_data_changed'));
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="flex items-center gap-1.5 px-2 py-1.5 sm:px-2.5 rounded-lg bg-orange-500/10 hover:bg-orange-500/20 text-orange-600 dark:text-orange-400 text-[10px] font-black transition-all cursor-pointer group/share"
                >
              ...
              >
                {activeCoupons.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                    <div className="w-12 h-12 bg-emerald-50 dark:bg-emerald-950/40 rounded-2xl flex items-center justify-center mb-3 border border-emerald-100 dark:border-emerald-900/40 shadow-xs">
                      <Ticket className="h-6 w-6 text-emerald-500/60 dark:text-emerald-400/60" />
                    </div>
                    <h4 className="text-sm font-black text-stone-700 dark:text-stone-300 mb-1">
                      현재 사용 가능한 쿠폰이 없습니다
                    </h4>
                    <p className="text-xs font-medium text-stone-400 dark:text-stone-500">
                      새로운 리딤코드가 발급되면 알려드릴게요!
                    </p>
                  </div>
                )}
              </motion.div>

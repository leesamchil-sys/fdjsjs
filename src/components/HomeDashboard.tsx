...
import { doc, setDoc, deleteDoc, updateDoc, serverTimestamp, collection, onSnapshot } from 'firebase/firestore';
...
const RingChart = ({ 
  ...
  const masterPct = total > 0 ? (master / total) * 100 : 0;
...
export default function HomeDashboard({
  ...
  const stats = useMemo(() => {
    ...
    return {
      birds: {
        master: visibleBirds.filter(b => masterBirdIds.has(b.id)).length,
        fiveStar: visibleBirds.filter(b => completedBirdIds.has(b.id) && ratings[b.name] === 5).length
      },
      insects: {
        master: visibleInsects.filter(i => masterInsectIds.has(i.id)).length,
        fiveStar: visibleInsects.filter(i => completedInsectIds.has(i.id) && ratings[i.name] === 5).length
      },
      fishing: {
        master: visibleFish.filter(f => masterFishIds.has(f.id)).length,
        fiveStar: visibleFish.filter(f => completedFishIds.has(f.id) && ratings[f.name] === 5).length
      },
      cooking: {
        master: visibleCooking.filter(c => masterFoodIds.has(c.id)).length,
        fiveStar: visibleCooking.filter(c => completedFoodIds.has(c.id) && ratings[c.name] === 5).length
      },
      gardening: {
        master: visibleFlowers.filter(g => masterGardeningIds.has(g.id)).length,
        fiveStar: visibleFlowers.filter(g => completedFlowerIds.has(g.id) && ratings[g.name] === 5).length
      },
      crops: {
        master: visibleCrops.filter(g => masterGardeningIds.has(g.id)).length,
        fiveStar: visibleCrops.filter(g => completedCropIds.has(g.id) && ratings[g.name] === 5).length
      }
    };
  }, [
    masterBirdIds, masterInsectIds, masterFishIds, masterFoodIds, masterGardeningIds,
    completedBirdIds, completedInsectIds, completedFishIds, completedFoodIds, completedFlowerIds, completedCropIds,
    ratings
  ]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(noticesRef, (snapshot) => {
      console.log(`[SNAPSHOT] notices - path: ${noticesRef.path}, size: ${snapshot.size}`);
      const items: Notice[] = [];
    ...
    <div className="max-w-[1240px] mx-auto w-full space-y-6 animate-fade-in">
          ...
          <section className="space-y-3">
            ...
            {viewMode === 'simple' ? (
              ...
              <div 
                onClick={() => setActiveCategory?.('birds')}
                className="bg-amber-50/40 hover:bg-amber-100/40 dark:bg-amber-950/10 dark:hover:bg-amber-950/20 border border-amber-200 dark:border-amber-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      <div className="flex items-baseline gap-1">
                        <span className="text-[11px] md:text-xs text-stone-500 dark:text-stone-400 font-bold mr-1">전체</span>
                        <span className="text-xl md:text-2xl font-mono font-black text-stone-900 dark:text-stone-100 leading-none">
                          {completedBirdIds.size}
                        </span>
                        <span className="text-[11px] md:text-xs text-stone-450 dark:text-stone-500 font-extrabold">
                          / {birdTotal}
                        </span>
                      </div>
                      <div className="h-px bg-stone-200/60 dark:bg-stone-800/40 my-1.5 w-full" />
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-orange-600 dark:text-orange-300">
                            <Star className="h-2.5 w-2.5 fill-orange-500 text-orange-500 shrink-0" />
                            <span>명인 완료</span>
                          </div>
                          <div className="font-mono text-[11px] md:text-xs font-black text-orange-600 dark:text-orange-300">
                            {stats.birds.master} <span className="text-[9px] text-stone-400 font-bold">/ {birdTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                            <Star className="h-2.5 w-2.5 fill-purple-500 text-purple-500 shrink-0" />
                            <span>5성 완료</span>
                          </div>
                          <div className="font-mono text-[11px] md:text-xs font-black text-purple-600 dark:text-purple-300">
                            {stats.birds.fiveStar} <span className="text-[9px] text-stone-400 font-bold">/ {birdTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>

                    {/* Mobile & Tablet Layout (less than md) */}
                    <div className="block md:hidden w-full text-left">
                      <div className="flex items-baseline mt-1 sm:mt-1.5">
                        <span className="text-[10px] sm:text-xs font-bold text-stone-500 dark:text-stone-400 mr-1.5">전체</span>
                        <span className="text-xl sm:text-2xl font-mono font-black text-stone-900 dark:text-stone-100">
                          {completedBirdIds.size}
                        </span>
                        <span className="text-xs sm:text-sm font-bold text-stone-400 dark:text-stone-500 ml-1.5">
                          / {birdTotal}
                      ...
                      <div className="h-px bg-amber-100/70 dark:bg-amber-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          ...
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.birds.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {birdTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
              ...
              <div 
                onClick={() => setActiveCategory?.('insects')}
                className="bg-emerald-50/40 hover:bg-emerald-100/40 dark:bg-emerald-950/10 dark:hover:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                <div className="flex justify-between items-start gap-1">
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      ...
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-orange-600 dark:text-orange-300">
                            {stats.insects.master} <span className="text-[9px] text-stone-400 font-bold">/ {insectTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-purple-600 dark:text-purple-300">
                            {stats.insects.fiveStar} <span className="text-[9px] text-stone-400 font-bold">/ {insectTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="block md:hidden w-full text-left">
                      ...
                      <div className="h-px bg-emerald-100/70 dark:bg-emerald-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          ...
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.insects.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {insectTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-purple-600 dark:text-purple-300">
              ...
              <div 
                onClick={() => setActiveCategory?.('fishing')}
                className="bg-blue-50/40 hover:bg-blue-100/40 dark:bg-blue-950/10 dark:hover:bg-blue-950/20 border border-blue-200 dark:border-blue-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                <div className="flex justify-between items-start gap-1">
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      ...
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-orange-600 dark:text-orange-300">
                            {stats.fishing.master} <span className="text-[9px] text-stone-400 font-bold">/ {fishTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-purple-600 dark:text-purple-300">
                            {stats.fishing.fiveStar} <span className="text-[9px] text-stone-400 font-bold">/ {fishTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="block md:hidden w-full text-left">
                      ...
                      <div className="h-px bg-blue-100/70 dark:bg-blue-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-blue-600 dark:text-blue-455">
                            <Star className="h-3 w-3 fill-blue-500 text-blue-500 shrink-0" />
                            <span>명인</span>
                          </div>
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.fishing.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {fishTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-purple-600 dark:text-purple-300">
              ...
              <div 
                onClick={() => setActiveCategory?.('cooking')}
                className="bg-pink-50/40 hover:bg-pink-100/40 dark:bg-pink-950/10 dark:hover:bg-pink-950/20 border border-pink-200 dark:border-pink-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                <div className="flex justify-between items-start gap-1">
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      ...
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-orange-600 dark:text-orange-300">
                            {stats.cooking.master} <span className="text-[9px] text-stone-400 font-bold">/ {cookingTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-purple-600 dark:text-purple-300">
                            {stats.cooking.fiveStar} <span className="text-[9px] text-stone-400 font-bold">/ {cookingTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="block md:hidden w-full text-left">
                      ...
                      <div className="h-px bg-pink-100/70 dark:bg-pink-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-indigo-600 dark:text-indigo-455">
                            <Star className="h-3 w-3 fill-indigo-500 text-indigo-500 shrink-0" />
                            <span>명인</span>
                          </div>
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.cooking.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {cookingTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-purple-600 dark:text-purple-300">
              ...
              <div 
                onClick={() => setActiveCategory?.('gardening', 'flower')}
                className="bg-rose-50/40 hover:bg-rose-100/40 dark:bg-rose-950/10 dark:hover:bg-rose-950/20 border border-rose-200 dark:border-rose-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                <div className="flex justify-between items-start gap-1">
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      ...
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-orange-600 dark:text-orange-300">
                            {stats.gardening.master} <span className="text-[9px] text-stone-400 font-bold">/ {gardeningTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[11px] md:text-xs font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                          ...
                          <div className="font-mono text-[11px] md:text-xs font-black text-purple-600 dark:text-purple-300">
                            {stats.gardening.fiveStar} <span className="text-[9px] text-stone-400 font-bold">/ {gardeningTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="block md:hidden w-full text-left">
                      ...
                      <div className="h-px bg-rose-100/70 dark:bg-rose-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          ...
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.gardening.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {gardeningTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-purple-600 dark:text-purple-300">
              ...
              <div 
                onClick={() => setActiveCategory?.('gardening', 'crop')}
                className="bg-lime-50/40 hover:bg-lime-100/40 dark:bg-lime-950/10 dark:hover:bg-lime-950/20 border border-lime-200 dark:border-lime-900/60 p-2 sm:p-4 rounded-2xl transition-all shadow-xs flex flex-col justify-between cursor-pointer active:scale-98 select-none"
              >
                <div className="flex justify-between items-start gap-1">
                ...
                <div className="mt-2.5 md:mt-3.5 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4 w-full">
                  ...
                  <div className="flex-1 w-full min-w-0">
                    ...
                    <div className="hidden md:block w-full text-left">
                      <div className="flex items-baseline gap-1">
                        <span className="text-xs md:text-sm text-stone-500 dark:text-stone-400 font-bold mr-1">전체</span>
                        <span className="text-xl md:text-2xl font-mono font-black text-stone-900 dark:text-stone-100 leading-none">
                          {completedCropIds.size}
                        </span>
                        <span className="text-xs md:text-sm text-stone-455 dark:text-stone-500 font-extrabold">
                          / {cropTotal}
                        </span>
                      </div>
                      <div className="h-px bg-stone-200/60 dark:bg-stone-800/40 my-1.5 w-full" />
                      <div className="flex flex-col gap-1 w-full mt-1.5">
                        <div className="flex items-center justify-between text-xs md:text-sm font-bold">
                          ...
                          <div className="font-mono text-xs md:text-sm font-black text-orange-600 dark:text-orange-300">
                            {stats.crops.master} <span className="text-[10px] text-stone-400 font-bold">/ {cropTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-xs md:text-sm font-bold">
                          <div className="flex items-center gap-0.5 text-purple-600 dark:text-purple-300">
                          ...
                          <div className="font-mono text-xs md:text-sm font-black text-purple-600 dark:text-purple-300">
                            {stats.crops.fiveStar} <span className="text-[10px] text-stone-400 font-bold">/ {cropTotal}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="block md:hidden w-full text-left">
                      ...
                      <div className="h-px bg-lime-100/70 dark:bg-lime-900/30 my-2 w-full" />
                      <div className="flex flex-col gap-1.5 w-full mt-1.5">
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-lime-600 dark:text-lime-455">
                            <Star className="h-3 w-3 fill-lime-500 text-lime-500 shrink-0" />
                            <span>명인</span>
                          </div>
                          <div className="font-mono text-stone-900 dark:text-stone-100 font-extrabold text-[10px] sm:text-xs">
                            {stats.crops.master} <span className="text-stone-400 dark:text-stone-500 font-bold">/ {cropTotal}</span>
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-[10px] sm:text-xs font-bold">
                          <div className="flex items-center gap-1 text-purple-600 dark:text-purple-300">
              ...
              {localPets.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center py-2 text-center">
                  ...
                  {setActiveCategory && (
                    <button
                      onClick={() => setActiveCategory('petfood')}
                      className="mt-2 text-[10px] font-black text-stone-600 dark:text-stone-400 hover:underline cursor-pointer"
                    >
                      마이 펫 등록하기 ➔
                    </button>
                  )}

...
import { getServerTimeMs, getServerTimeKST, getKSTDateFromMs, syncServerTime, subscribeServerTimeSync } from '../utils/serverTime';
...
interface CropTimerProps {
  ...
  onCropCompleted?: (newlyCompletedSlots: any[]) => void;
}

// 5성 작물 단계 라벨 컴포넌트
const FiveStarStageLabel = ({ slot }: { slot: PlantedSlot }) => {
...
// 5성 작물 시간 티커 컴포넌트
// 5성 작물 시간 티커 컴포넌트
const FiveStarTicker = ({ slot }: { slot: PlantedSlot }) => {
...
export default function CropTimer({ 
  ...
  onSyncError, 
  onLogout,
  isInitialSyncDone: isGlobalSyncDone = false,
  isActive = true,
  cropPresets,
  ...
  const userPresetsRef = useRef<CropPreset[]>([]);
  ...
  const [currentTime, setCurrentTime] = useState(getServerTimeMs());
...
function sendOfflineTelegramNotification(token, chatId, triggeringSlot, allSlots, currentTime, isPre, stageId, preAlarmMinutes) {
  ...
  useEffect(() => {
    ...
    const handleFocus = () => syncServerTime();
  ...
  useEffect(() => {
    ...
    if (!user) {
      ...
      // CLEAR local guest storage on logout
    ...
    const runInitialSync = async () => {
      try {
        const sessionKey = `farming_fetched_${user.uid}`;
        const hasFetchedThisSession = sessionStorage.getItem(sessionKey) === 'true';

        if (hasFetchedThisSession) {
          console.log("[Sync] Already fetched farming slots this session. Loading from localStorage instead of getDoc.");
          ...
          const savedPresets = localStorage.getItem('user_notification_presets');
          if (savedPresets) {
            setUserPresets(safeJsonParse(savedPresets, []));
          }
          const savedSound = localStorage.getItem('farming_sound_enabled');
          if (savedSound) {
            setSoundEnabled(safeJsonParse(savedSound, true));
          }
          return;
        }
    ...
    const subscribeToRealtimeUpdates = () => {
      gardenUnsubscribe = onSnapshot(userDocRef, async (snapshot) => {
        ...
        try {
          ...
          if (isFarmingInitialSyncDoneRef.current && farmingSyncTimerRef.current !== null) {
            return;
          }
          ...
          if (snapshot.exists()) {
            ...
            if (isFarmingInitialSyncDoneRef.current && cloudUpdatedAt > 0 && cloudUpdatedAt < globalLock) {
              return;
            }
  ...
  const syncFarmingStateToFirestore = async (newSlots: PlantedSlot[], oldSlots: PlantedSlot[] = []) => {
    ...
    try {
      ...
      if (Object.keys(farmingSlotsPayload).length > 0) {
        // Compare with old slots hash to prevent unnecessary writes
        const oldSlotsJson = JSON.stringify(oldSlots.map(s => ({...s, updatedAt: undefined})));
        const newSlotsJson = JSON.stringify(newSlots.map(s => ({...s, updatedAt: undefined})));
        
        if (oldSlotsJson === newSlotsJson) {
          console.log("[Farming Sync] Data actually unchanged, skipping write.");
          return;
        }
  ...
  const triggerDebouncedFarmingSync = (newSlots: PlantedSlot[]) => {
    ...
    const preferredDelay = 2000;
    let actualDelay = preferredDelay;
    if (typeof getGlobalSyncRemainingTime === 'function') {
      const remainingGlobal = getGlobalSyncRemainingTime();
      if (remainingGlobal !== null && remainingGlobal > 0) {
        actualDelay = Math.min(remainingGlobal, preferredDelay);
      }
    }

    const targetTime = Date.now() + actualDelay;
    onFarmingSyncScheduled?.(targetTime, true);
    ...
    farmingSyncTimerRef.current = setTimeout(async () => {
      ...
      onFarmingSyncScheduled?.(0, false);
      if (typeof debouncedSyncAllData === 'function') {
  ...
  useEffect(() => {
    const forceFlushFarmingSync = () => {
      if (farmingSyncTimerRef.current) {
        clearTimeout(farmingSyncTimerRef.current);
        farmingSyncTimerRef.current = null;
        if (user && isGlobalSyncDone && isFarmingInitialSyncDoneRef.current) {
          console.log("[Farming Sync] Force flushing slots due to page exit...");
          const newSlots = slotsRef.current;
          const oldSlots = lastFarmingSyncSlotsRef.current;
          syncFarmingStateToFirestore(newSlots, oldSlots).catch(err => {
            console.warn("[Farming Sync] Force flush failed:", err);
          });
          lastFarmingSyncSlotsRef.current = newSlots;
          onFarmingSyncScheduled?.(0, false);
        }
      ...
      if (presetsSyncTimerRef.current) {
        clearTimeout(presetsSyncTimerRef.current);
        presetsSyncTimerRef.current = null;
        if (user && isGlobalSyncDone) {
          console.log("[Farming Sync] Force flushing presets due to page exit...");
          const presetsStr = JSON.stringify(userPresetsRef.current);
          if (presetsStr !== lastSyncedPresetsRef.current) {
            const userDocRef = doc(db, 'users', user.uid);
            setDoc(userDocRef, { userPresets: JSON.parse(presetsStr) }, { merge: true }).catch(() => {});
            lastSyncedPresetsRef.current = presetsStr;
          }
        }
      }
    ...
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (farmingSyncTimerRef.current || presetsSyncTimerRef.current) {
        forceFlushFarmingSync();
      }
    };
    ...
    const handleAppForceSync = () => {
      forceFlushFarmingSync();
    };

    window.addEventListener('beforeunload', handleBeforeUnload, { capture: true });
    document.addEventListener('visibilitychange', handleVisibilityChange);
    window.addEventListener('app-force-sync-before-reload', handleAppForceSync);

    return () => {
      window.removeEventListener('beforeunload', handleBeforeUnload, { capture: true });
      document.removeEventListener('visibilitychange', handleVisibilityChange);
      window.removeEventListener('app-force-sync-before-reload', handleAppForceSync);
    };
  }, [user, isGlobalSyncDone]);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (newlyCompletedSlots.length > 0) {
        if (!isInitialTick) {
          onCropCompleted?.(newlyCompletedSlots);
        }
        if (isInitialTick) {
    ...
    return () => clearInterval(timer);
  }, [telegramToken, telegramChatId, soundEnabled, user, timeOffset, notificationPermission, isActive]);

  // Update currentTime immediately when crops tab becomes active
  useEffect(() => {
    if (isActive) {
              ...
              <div className="p-5 md:p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4">
                  ...
                  <div
                    ...
                    className={cn(
                      "relative group cursor-pointer p-2.5 sm:p-4 flex flex-col items-center justify-center gap-2 sm:gap-3 bg-white dark:bg-stone-900 active:scale-[0.98] card-layout",
                      soundEnabled 
                        ? 'border-amber-300 dark:border-amber-400/50 bg-amber-50 dark:bg-amber-400/10 hover:border-amber-400 dark:hover:border-amber-300 shadow-sm' 
                        : 'card-border-style'
                    )}
                  >
                    <button
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      notificationPermission === 'granted'
                        ? 'border-emerald-300 dark:border-emerald-400/50 bg-emerald-50 dark:bg-emerald-400/10 hover:border-emerald-400 dark:hover:border-emerald-300 shadow-sm'
                        : 'card-border-style'
                    )}
                  >
                    <button
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      (telegramToken && telegramChatId)
                        ? 'border-emerald-300 dark:border-emerald-400/50 bg-emerald-50 dark:bg-emerald-400/10 hover:border-emerald-400 dark:hover:border-emerald-300 shadow-sm'
                        : 'card-border-style hover:border-slate-400 dark:hover:border-stone-500'
                    )}
                  >
                    <button
                      type="button"
                      ...
                      <div className="bg-neutral-50 dark:bg-stone-850 rounded-2xl border border-neutral-200 dark:border-stone-800 p-4 md:p-6 space-y-6 mt-4 animate-fadeIn">
                        ...
                        {tgTab === 'basic' ? (
                          <div className="space-y-4">
                            ...
                            <div className="p-4 space-y-3 flex items-center justify-between base-card-style">
        ...
        {slots.map((slot, index) => {
          ...
          if (isPlanted) {
            if (isFruited) {
              ...
              stateStyle = slot.isFiveStarMode
                ? 'bg-gradient-to-br from-amber-50 via-yellow-50/70 to-orange-50 border-amber-400 ring-4 ring-amber-500/20 text-amber-950 dark:from-amber-950/45 dark:via-yellow-950/10 dark:to-orange-950/25 dark:border-amber-700 dark:ring-amber-950/35 dark:text-amber-200'
                : 'bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-400 ring-4 ring-emerald-500/10 text-emerald-900 dark:from-emerald-950/20 dark:to-teal-950/20 dark:border-emerald-800 dark:ring-emerald-950/15 dark:text-emerald-300';
            } else if (percent < 30) {
              stageLabel = '진행 중';
              stageEmoji = slot.cropEmoji || '🌱';
              stateStyle = slot.isFiveStarMode
                ? 'bg-gradient-to-br from-amber-50/20 to-white border-amber-300/80 dark:from-amber-950/10 dark:to-stone-900 dark:border-amber-800/80 dark:text-stone-100'
                : 'bg-white border-neutral-200 dark:bg-stone-900 dark:border-stone-800 dark:text-stone-100';
            } else if (percent < 65) {
              stageLabel = '진행 중';
              stageEmoji = slot.cropEmoji || '🌿';
              stateStyle = slot.isFiveStarMode
          ...
          return (
            <div 
              key={slot.id} 
              className={cn(
                "relative overflow-hidden p-4 pb-4 transition-all flex flex-col justify-between min-h-[300px] h-auto card-layout",
                stateStyle
              )}
            >
                    ...
                    <div className="space-y-1 relative">
                      <div className="flex justify-between gap-4 text-[11px] font-mono font-medium text-neutral-500 dark:text-stone-400 h-5 items-center px-0.5">
                        <div className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-tighter">
                          {slot.isFiveStarMode && !isFruited && <FiveStarStageLabel slot={slot} />}
                        </div>
                        <div className="flex-1 flex justify-end">
                          <AnimatePresence mode="wait">
                            {!isFruited && (
                              slot.isFiveStarMode ? (
                                <FiveStarTicker slot={slot} />
                              ) : (
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-5 pr-1 font-sans">
                ...
                {tgTab === 'basic' ? (
                  <div className="space-y-4 text-left">
                    ...
                    <div className="p-4 flex items-center justify-between base-card-style">
                  ...
                  <div className="space-y-5 text-left animate-fadeIn">
                    {/* High Quality visual step guide */}
                    <div className="p-4 md:p-5 space-y-4 base-card-style">
...
function formatCompletionTime(timestamp: number, showLabel: boolean, isCompleted: boolean): string {
  // Use local Date for the completion timestamp so wall-clock time matches user's local timezone
  ...
  // Base 'now' on accurate server time epoch, formatted in local timezone
  const now = new Date(getServerTimeMs());

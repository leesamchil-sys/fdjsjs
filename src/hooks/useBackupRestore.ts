...
import { getAllPetImagesAsDataUrls, savePetImagesFromDataUrls } from '../utils/indexedDB';
...
export function useBackupRestore({
  ...
  const handleBackupData = async (): Promise<string> => {
    let petImageDataUrls: Record<string, string> = {};
    try {
      petImageDataUrls = await getAllPetImagesAsDataUrls();
    } catch (err) {
      console.error('[Backup] Failed to fetch pet image data URLs:', err);
    }
    ...
    const backup = {
      ...
      pigtown_pets: pets,
      pet_image_data_urls: petImageDataUrls,
  ...
  const handleRestoreData = async (data: any) => {
    ...
    // Restore pet images into IndexedDB if present in backup JSON
    if (data.pet_image_data_urls && typeof data.pet_image_data_urls === 'object') {
      try {
        await savePetImagesFromDataUrls(data.pet_image_data_urls);
      } catch (err) {
        console.error('[Restore] Failed to restore pet images from backup:', err);
      }
    }
    ...
    const rawPets = Array.isArray(data.pigtown_pets) ? data.pigtown_pets : [];
    const restoredPets = rawPets.map((p: any) => {
      if (!p || typeof p !== 'object') return p;
      const hasImage = !!(p.hasCustomImage || (data.pet_image_data_urls && data.pet_image_data_urls[p.id]));
      return {
        ...p,

...
import { doc, getDoc, setDoc, serverTimestamp, deleteField, getDocFromServer } from 'firebase/firestore';
...
import { migrateItemRatings, OCEAN_RENAME_MAP } from '../lib/appHelpers';
...
export function useUserDataSync(user: any) {
  ...
  const [ratings, setRatings] = useState<Record<string, number>>(() => {
    ...
    return migrateItemRatings(safeJsonParse(saved, {}));
  });
  ...
  function applyFetchedDataToLocal(fields: any, loginUid: string) {
    ...
    localStorage.setItem('local_collections_updated_at', nowStr);
    localStorage.setItem('collections_write_lock_at', nowStr);
  ...
  async function runInitialSync(loggedInUser: any) {
    ...
    try {
      ...
      const docSnap = await Promise.race([
        ...
        new Promise<any>((_, reject) => setTimeout(() => reject(new Error("network-timeout")), 3500))
      ]);
      ...
      const isMeaningfulPet = (p: any) => {
        if (!p || typeof p !== 'object') return false;
        if (p.id || p.name) return true;
        return false;
      };
      const hasLocalPetsProgress = localPets.some(isMeaningfulPet);

      const hasLocalProgress = localBirds.size > 0 || localInsects.size > 0 || localFish.size > 0 || localFood.size > 0 ||
        localGardening.size > 0 || localOceanCleaning.size > 0 ||
        localMasterBirds.size > 0 || localMasterInsects.size > 0 || localMasterFish.size > 0 || localMasterFood.size > 0 ||
        localMasterGardening.size > 0 || localMasterOceanCleaning.size > 0 ||
        Object.keys(localRatings).length > 0 || hasLocalPetsProgress ||
      ...
      if (docSnap.exists()) {
        ...
        const cloudOceanCleaningList = (data.completedOceanCleaningNames || []).map((name: string) => {
          const mappedName = OCEAN_RENAME_MAP[name] || name;
          return ALL_OCEAN_CLEANING_MAP.find(o => o.name === mappedName || o.id === mappedName)?.id || mappedName;
        });
        ...
        const cloudMasterOceanCleaningList = (data.masterOceanCleaningNames || []).map((name: string) => {
          const mappedName = OCEAN_RENAME_MAP[name] || name;
          return ALL_OCEAN_CLEANING_MAP.find(o => o.name === mappedName || o.id === mappedName)?.id || mappedName;
        });
        ...
        const cloudRatings = migrateItemRatings(data.ratings || {});
        ...
        const arePetsEqual = (a: any[], b: any[]): boolean => {
          ...
          for (let i = 0; i < listA.length; i++) {
            ...
            if (petA.id !== petB.id) return false;
            if (petA.name !== petB.name) return false;
            if (!!petA.isHotel !== !!petB.isHotel) return false;
            if (petA.type !== petB.type) return false;
            if (!!petA.hasCustomImage !== !!petB.hasCustomImage) return false;
            if (!areValuesEqual(petA.preferences || {}, petB.preferences || {})) return false;
            if (!areValuesEqual(petA.tried || {}, petB.tried || {})) return false;
          }
          return true;
        };

        const areSlotsEqual = (a: any[], b: any[], reasons?: string[]): boolean => {
          const listA = (a || []).map(s => s || {});
          const listB = (b || []).map(s => s || {});
          
          if (listA.length !== listB.length) {
            reasons?.push(`슬롯 개수가 다릅니다. (로컬: ${listB.length}, 클라우드: ${listA.length})`);
            return false;
          }
          ...
          for (let i = 0; i < listA.length; i++) {
            const sa = listA[i];
            const sb = listB[i];
            const slotName = sa.cropName || sb.cropName || sa.id || `${i + 1}번 슬롯`;
            
            if (sa.id !== sb.id) {
              reasons?.push(`${slotName}의 ID가 다릅니다. (로컬: ${sb.id}, 클라우드: ${sa.id})`);
              return false;
            }
            if (sa.cropId !== sb.cropId) {
              reasons?.push(`${slotName}의 작물 ID가 다릅니다. (로컬: ${sb.cropId}, 클라우드: ${sa.cropId})`);
              return false;
            }
            if (sa.cropName !== sb.cropName) {
              reasons?.push(`${slotName}의 작물 이름이 다릅니다. (로컬: ${sb.cropName}, 클라우드: ${sa.cropName})`);
              return false;
            }
            if (sa.cropEmoji !== sb.cropEmoji) {
              reasons?.push(`${slotName}의 작물 이모지가 다릅니다. (로컬: ${sb.cropEmoji}, 클라우드: ${sa.cropEmoji})`);
              return false;
            }
            ...
            if (saStart !== sbStart) {
              reasons?.push(`${slotName}의 시작 시간이 다릅니다. (로컬: ${sbStart}, 클라우드: ${saStart})`);
              return false;
            }
            ...
            if (saDuration !== sbDuration) {
              reasons?.push(`${slotName}의 소요 시간이 다릅니다. (로컬: ${sbDuration}, 클라우드: ${saDuration})`);
              return false;
            }

            if ((sa.userOffset || 0) !== (sb.userOffset || 0)) {
              reasons?.push(`${slotName}의 오프셋이 다릅니다. (로컬: ${sb.userOffset || 0}, 클라우드: ${sa.userOffset || 0})`);
              return false;
            }
            if (!!sa.isFiveStarMode !== !!sb.isFiveStarMode) {
              reasons?.push(`${slotName}의 5성 모드 여부가 다릅니다. (로컬: ${!!sb.isFiveStarMode}, 클라우드: ${!!sa.isFiveStarMode})`);
              return false;
            }
            if (!!sa.isNotified !== !!sb.isNotified) {
              reasons?.push(`${slotName}의 알림 여부가 다릅니다. (로컬: ${!!sb.isNotified}, 클라우드: ${!!sa.isNotified})`);
              return false;
            }
          }
          return true;
        ...
        const slotDiffReasons: string[] = [];
        if (!areSlotsEqual(cloudSlotsList, localSlotsList, slotDiffReasons)) {
          diffFields.push("농장 및 작물 상태 (" + slotDiffReasons.join(", ") + ")");
        }
      ...
      if (errStr.includes("timeout") || errStr.includes("network") || errStr.includes("offline") || errStr.includes("failed to get")) {

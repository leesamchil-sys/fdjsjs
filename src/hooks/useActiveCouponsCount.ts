...
import { getServerTimeKST, getServerTimeMs } from '../utils/serverTime';
  ...
  useEffect(() => {
    ...
    const fetchCouponsCount = async () => {
      try {
        ...
        const isCacheValid = cacheTime && cachedData && (getServerTimeMs() - parseInt(cacheTime, 10) < 3600000); // 1시간 캐시

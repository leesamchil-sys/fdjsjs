 ...
 */
export function isSeasonOngoing(event: SeasonalEvent, now = new Date()): boolean {
 ...
 */
export function isSeasonEnded(event: SeasonalEvent, now = new Date()): boolean {
 ...
 */
export function getSeasonStatus(event: SeasonalEvent, now = new Date()): 'ongoing' | 'ended' | 'upcoming' {
  if (event.startDate) {
    const start = new Date(event.startDate + 'T00:00:00');

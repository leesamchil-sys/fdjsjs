...
const ALL_OCEAN_CLEANING_MAP = OCEAN_CLEANING_ITEMS;

const HeaderClock = memo(({ currentTime }: { currentTime: Date }) => {
  return (
...
export default function App() {
  ...
  const [activeCategory, _setActiveCategory] = useState<Category>(() => {
    ...
    const validCategories: Category[] = ['home', 'birds', 'insects', 'fishing', 'cooking', 'crops', 'petfood', 'gardening', 'ocean_cleaning', 'privacy', 'terms', 'coupons', 'trend_checklist'];
  ...
  const setActiveCategory = (cat: Category | ((prev: Category) => Category)) => {
    const newCategory = typeof cat === 'function' ? cat(activeCategory) : cat;
    // Immediate menu state sync for zero-delay sidebar highlight
    _setActiveCategory(newCategory);
    localStorage.setItem('active_category', newCategory);
  ...
  // Disable browser automatic scroll restoration and reset scroll instantly before paint
  useEffect(() => {
  ...
  // Sync URL to state when location changes
  useEffect(() => {
    const path = location.pathname.split('/')[1] as Category;
    const validCategories: Category[] = ['home', 'birds', 'insects', 'fishing', 'cooking', 'crops', 'petfood', 'gardening', 'ocean_cleaning', 'privacy', 'terms', 'coupons', 'trend_checklist'];
    if (path && validCategories.includes(path) && path !== activeCategory) {
      startTransition(() => {
        _setActiveCategory(path);
      });
      localStorage.setItem('active_category', path);
    }
  }, [location.pathname]);
  ...
  // 파이어베이스에서 운영날씨 설정 단발성 조회 (브라우저 실행 또는 새로고침 시에만)
  useEffect(() => {
  ...
  const isForceUpdateRequired = useMemo(() => {
    ...
    if (import.meta.env.DEV) {
      console.log(`[VersionCheck] Current: ${APP_VERSION}, MinRequired: ${minSupportedVersion}, IsOlder: ${isOlder}, ForceActive: ${isForceUpdateActive}`);
    }
    return isForceUpdateActive && isOlder;
  }, [minSupportedVersion, APP_VERSION, isForceUpdateActive]);
  ...
  const handleReportSubmit = async (reportType: 'bug' | 'info' | 'suggest', messageContent: string, files: File[], memberStatus: 'member' | 'non-member' | null) => {
    setIsContactModalOpen(false);
    
    // Show sending toast first
    ...
    // Send as multipart/form-data for clean multiple file upload support
  ...
  useEffect(() => {
    localStorage.setItem('flower_color_collections', JSON.stringify(flowerColorCollections));
  }, [flowerColorCollections]);

  const handleToggleFlowerColor = (itemId: string, variantKey: string) => {
    setFlowerColorCollections(prev => {
      const itemMap = prev[itemId] || {};
      const nextMap = { ...itemMap, [variantKey]: !itemMap[variantKey] };
      const next = { ...prev, [itemId]: nextMap };
      localStorage.setItem('flower_color_collections', JSON.stringify(next));
      return next;
    });
  ...
  // Determine if entering via map permalink on startup
  ...
  const [isWelcomeOpen, setIsWelcomeOpen] = useState(() => {
    ...
    // Delay welcome modal if entering via map permalink
  ...
  const handleLocationClick = (locationName: string, itemName: string) => {
    ...
    if (matched) {
      ...
      if (matchedKey) {
        ...
        navigate(`/map=${encodeURIComponent(urlMapId)}?location=${encodeURIComponent(locationId)}`, { replace: false });
        return;
      }
    }
    
    // Fallback if not matched
    navigate(`/map=${encodeURIComponent('고래섬')}`, { replace: false });
  };
  ...
  const globalSyncTargetTimeRef = useRef<number>(0);

  const getGlobalSyncRemainingTime = useCallback(() => {
    if (!globalSyncTimerRef.current) return null;
    const remaining = globalSyncTargetTimeRef.current - Date.now();
    return remaining > 0 ? remaining : null;
  }, []);

  const farmingSyncTargetTimeRef = useRef<number>(0);
  const hasFarmingSyncPendingRef = useRef<boolean>(false);

  const getFarmingSyncRemainingTime = useCallback(() => {
    if (!hasFarmingSyncPendingRef.current) return null;
    const remaining = farmingSyncTargetTimeRef.current - Date.now();
    return remaining > 0 ? remaining : null;
  }, []);

  const onFarmingSyncScheduled = useCallback((targetTime: number, isPending: boolean) => {
    farmingSyncTargetTimeRef.current = targetTime;
    hasFarmingSyncPendingRef.current = isPending;
  }, []);
    ...
    const activeUnsubscribe = onSnapshot(userDocRef, (snapshot) => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          ...
          const lockTime = parseInt(localStorage.getItem('collections_write_lock_at') || '0', 10);

          // Prevent passive sync from overwriting local edits if we recently made changes
          // or have unsynced changes specifically for collections.
          if (isDirtyRef.current) {
            console.log("[Sync] Local collection changes are pending. Skipping passive snapshot.");
            return;
          }
          
          if ((Date.now() - lockTime < 10000) && (cloudUpdatedAt <= localUpdatedAt + 500)) {
            console.log("[Sync] Recently locked, update not newer (echo). Skipping passive snapshot to prevent overwrite.");
            return;
          }
          ...
          if (JSON.stringify(cloudFlowerColors) !== JSON.stringify(localFlowerColors)) dataActuallyChanged = true;
          ...
          if (dataActuallyChanged) {
            ...
            setFlowerColorCollections(cloudFlowerColors);
            ...
            localStorage.setItem('flower_color_collections', JSON.stringify(cloudFlowerColors));
  ...
  function getWriteReason(prevData: any, currentData: any): string[] {
    ...
    if (JSON.stringify(prevData.flowerColorCollections) !== JSON.stringify(currentData.flowerColorCollections)) reasons.push("flowerColorCollectionsChanged");
  ...
  async function forceSyncAllData(currentUser: any, skipVerify: boolean = false): Promise<boolean> {
    ...
    try {
      // Capture that we are attempting a sync now
      isDirtyRef.current = false;
  ...
  function debouncedSyncAllData(delay: number = 30000) {
    if (!user) return;
    ...
    globalSyncTimerRef.current = setTimeout(async () => {
      ...
      try {
        const userDocRef = doc(db, 'users', user.uid);
        localStorage.setItem('local_last_write_at', Date.now().toString());
        // [방어벽 갱신] 30초 대기 후 실제 서버로 전송하는 시점에 10초 방어벽을 다시 가동하여 메아리 방지
        ...
        // [Race Condition 방어]
        // 비동기 통신(await setDoc)을 기다리는 수십~수백ms 동안, 유저가 다시 화면을 클릭해서 
        // 새로운 데이터가 발생(isDirtyRef.current === true)했다면 맹목적으로 초기화하지 않습니다.
        if (!isDirtyRef.current) {
          localStorage.removeItem('has_unsynced_changes');
          setHasUnsyncedChanges(false);
          window.dispatchEvent(new Event('sync-status-changed'));
  ...
  const handleRate = async (id: string | null, name: string, rating: number) => {
    ...
    setRatings(prev => {
      ...
      if (rating === 0) {
        ...
        // Find if item has maxStars limit
        let maxStars = 5;
        const allPossible = [
          ...dbBirds, ...dbInsects, ...dbFish, ...dbCooking, ...gardeningItems, ...oceanCleaning
        ];
        const matched = allPossible.find(i => i.name === name || (id && i.id === id));
        if (matched && matched.maxStars) {
          maxStars = matched.maxStars;
        }
        nextRatings[name] = Math.min(rating, maxStars);
      }
      localStorage.setItem('item_ratings', JSON.stringify(nextRatings));
      return nextRatings;
    });
  ...
  return (
    <div className="min-h-screen bg-stone-50/60 dark:bg-stone-950/90 flex flex-col relative transition-colors duration-300">
      ...
      >
        ...
        <div className="px-4 sm:px-5 md:px-6 pt-4 pb-6 relative bg-stone-50 dark:bg-stone-950 transition-colors shadow-[0_-1px_3px_rgba(0,0,0,0.02)] flex-1 flex flex-col">
          <CategoryView 
            activeCategory={activeCategory}
      ...
      {(() => {
        ...
        const handleRestoreData = (data: any) => {
          ...
          // Notify components of trend checklist changes
          ...
          // Notify map component to refresh its state if open
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="fixed bottom-24 left-1/2 z-[200] flex items-center gap-2.5 px-5 py-3.5 bg-neutral-900 border border-neutral-800 text-white rounded-full text-xs font-bold shadow-2xl backdrop-blur-md max-w-[90vw] md:max-w-md text-left justify-start"
          >
            {toastType === 'error' ? (
      ...
      <FloatingReportButton 
        ...
        setIsMapOpen={(open) => {
          if (open) {
            ...
            navigate(`/map=${encodeURIComponent(mapName)}`);
          } else {
            const newPathname = '/' + activeCategory;
            const params = new URLSearchParams(window.location.search);
            params.delete('map');

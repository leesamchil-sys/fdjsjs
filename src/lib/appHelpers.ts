...
const cookingPng = new Set([
  ...
  "딸기 잼 오징어구이", "블루베리 잼 오징어구이", "사과 잼 오징어구이", "스타프루트 잼 오징어구이", "파인애플 잼 오징어구이"
]);

export function getExistingImagePath(type: Category, name: string): string | null {
  ...
  if (type === 'birds') {
    if (birdWebp.has(cleanName)) {
      return `/images/${subfolder}/${cleanName}.webp`;
    }
    if (birdPng.has(cleanName)) {
      return `/images/${subfolder}/${cleanName}.png`;
    }
  } else if (type === 'insects') {
    if (insectPng.has(cleanName)) {
      return `/images/${subfolder}/${cleanName}.png`;
    }
  } else if (type === 'fishing') {
    if (fishPng.has(cleanName)) {
      return `/images/${subfolder}/${cleanName}.png`;
    }
    ...
    if (cookingPng.has(lookupName)) {
      return `/images/${subfolder}/${lookupName}.png`;
    }
